<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet 
    xmlns:tei="http://www.tei-c.org/ns/1.0"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    <xsl:param name="project_title">Tillich Korrespondenz</xsl:param>
    <xsl:param name="project_short_title">Tillich Briefe</xsl:param>
    <xsl:param name="github_url">https://github.com/TillichCorrespondence/tillich-briefe-static</xsl:param>
    <xsl:param name="html_title">Tillich Briefe</xsl:param>
    <xsl:param name="project_logo">images/fundament_logo.svg</xsl:param>
    <xsl:param name="base_url">https://tillich-briefe.acdh.oeaw.ac.at</xsl:param>
</xsl:stylesheet>